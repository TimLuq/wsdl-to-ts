...
Promise.all(config.files.map(uri => wsdl_to_ts_1.wsdl2ts(uri, wsdl_options, opts)))
        ...
        return mkdirpp(dir).then(() => {
            return new Promise((resolve, reject) => {
                ...
                if (config.tslintEnable === null) {
                    fileData.push('/* tslint:enable */' + os_1.EOL);
                }
                if (config.tslintDisable === null) {
                    fileData.push('/* tslint:disable */' + os_1.EOL);
                }
                ...
                fs_1.writeFile(tsfile, formattedFile, err => {
                    if (err) {
                        reject(err);
                    }
                    else {
