...
function outputTypedWsdl(a, outputConfig) {
    const r = [];
    for (const service of Object.keys(a.files)) {
        for (const port of Object.keys(a.files[service])) {
            ...
            const relativeWsdl = path.relative(outputConfig.wsdlImportBasePath ? outputConfig.wsdlImportBasePath : absoluteServiceFile, absoluteWsdl);
            ...
            interfaceFile.data.push(`import { XmlNamespace, XmlOrder } from "common/lib-soapclient/src/lib/wsdl.decorators";`);
            ...
            if (a.methods[service] && a.methods[service][port]) {
                const ms = [];
                serviceFile.data.push(template_1.default.serviceHeaderTemplate({
                    relativeTypesPath,
                    serviceName: service,
                ...
                for (const method of Object.keys(a.methods[service][port])) {
                    ...
                    serviceFile.data.unshift(template_1.default.serviceImportTemplate(templateObj));
                    serviceFile.data.push(template_1.default.serviceMethodTemplate(templateObj));
                }
